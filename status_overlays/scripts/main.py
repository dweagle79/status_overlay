...
script_directory = os.path.join(main_directory, "scripts")

def log_setup():
    #Setup log formatter and handler with rotation
    
    # Define the log directory inside the script directory
    log_directory = os.path.join(main_directory, "logs")  # Logs folder inside the script directory
...
def main():
    try:
        settings_file_path = os.path.join(main_directory, "overlay-settings.yaml")
        ...
        if not os.path.exists(settings_file_path):
            ...
            logger.info("Please edit the 'overlay-settings.yaml' to your preffered Kometa settings and rerun the script.")
